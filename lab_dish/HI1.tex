...
class (May 1). All of the HI projects involve making maps and
...
report. Report is due the last day of class, 1 May.
\end{enumerate}
...
For running observing sessions that run automatically for long periods of
...
We also have an l.o. that you can shift around for calibration purposes.  It's actually the same
Agilent l.o. you used in the horn lab, only we've unplugged it and moved it to Leuschner.  This
means you can control it with {\it ugradio.agilent}, same as before, but connect to the {\tt heiles}
computer, instead of the lab machine that was hosting it.  Since {\tt heiles} is the computer you are
already logged into, use the address 127.0.0.1 (which means ``self").
\begin{enumerate}
\item {\tt ugradio.agilent.SynthClient(host='127.0.0.1')} instantiates an interface to the l.o.
\item {\tt SynthClient.set\_frequency} changes the l.o. frequency.
\end{enumerate}
