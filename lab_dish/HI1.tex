...
structures that vary greatly in size---from tiny self-gravitating clouds
to galactic-scale spiral arms.
...
starlight, and by line cooling in dense, shielded molecular clouds where new stars form.
...
\begin{verbatim}
   ssh radiolab@leuschner.berkeley.edu
\end{verbatim}
\noindent
...
If you, like me, get tired of manually multi-hopping through computers to get to the one you want,
...
time, we will use \texttt{screen} to serve up a remote desktop. \texttt{screen} is a command that allows us to maintain a persistent session on a remote machine even when everyone is logged out. If everyone uses the same \texttt{screen} session, we avoid having more than one group observing at a time.
We have set up a \texttt{screen} window on the Pi at Leuschner.  

To attach to this window, run 
\begin{verbatim}
    screen -r
\end{verbatim} 
...
\item {\tt LeuschTelescope.point(alt,az)} moves the telescope to a specified alt, az in degrees.
...
\begin{enumerate}
\item {\tt ugradio.leusch.LeuschNoise()} instantiates an interface to the noise diode (controlled
by a Raspberry Pi).
...
\subsubsection{Option 1 (preferred): Using SDRs}

Because we are using the Leuschner dish to observe the 21-cm line, we don't need much spectral bandwidth,
which means we can use the same SDRs you used in Lab 2. We have plugged two separate SDRs into the same Pi,
so if you would like to observe with both feed polarizations, you can build two separate SDR interfaces.
Use the \texttt{device\_index} keyword to select which SDR to target:
\begin{verbatim}
    >>> sdr0 = SDR(device_index=0)
    >>> sdr1 = SDR(device_index=1)
\end{verbatim}

Recording raw time samples will quickly use up a lot of disk space, so we suggest that,
after setting an appropriate \texttt{gain}, \texttt{center\_freq} and \texttt{sample\_rate}, you compute and record
averaged power spectra. Choose an appropriate number of samples to FFT to give yourself plenty of frequency resolution
for fitting Gaussian profiles to your HI spectra, but not so much that you waste disk space/sensitivity.
Remember that each SDR must be configured separately, and that they are running off of separate
oscillators, so they will be slightly out of synchronization. This is okay, because we are not cross-correlating them.

\subsubsection{Option 2: Using the SNAP}

A second option is to use the same SNAP board we used in Lab 3, but as a spectrometer.
This spectrometer is the same FPGA design as the interferometer correlator, but we will initialize it with
\texttt{mode='spec'} to deliver us two auto-correlations instead of a cross-correlation. First, though, we need
wo mix the 21-cm line down to be sampled by the SNAP. We have provided an LO you can control
remotely with {\it ugradio.agilent}. 
\begin{enumerate}
...
\end{enumerate}
\noindent
The output of this LO goes through a 30-MHz bandpass filter centered at 327 MHz, so set your LO frequency accordingly.

Next, we need configure the spectrometer to operate with a slower sample clock, so that we can spread our 1024
channels over a narrow bandwidth to get better (but still probably too coarse) frequency resolution.
This is done as follows:
\texttt{mode='spec'} to deliver us two auto-correlations instead of the cross-correlation, e.g.:
\begin{verbatim}
    >>> import snap_spec
    >>> spec = snap_spec.snap.UGRadioSnap(is_discover=True, acc_len=6000)
    >>> target_sample_rate = 77 # MHz
    >>> spec.initialize(mode='spec', sample_rate=target_sample_rate)
    >>> actual_sample_rate = spec.adc.lmx.getFreq()
\end{verbatim}
\noindent
You will see, running the commands above, that the actual sample rate is not quite equal to the target sample rate. Make
sure to use the actual one in you frequency calculations. Note that, for a $\sim77$ MHz sample rate, we will be sampling a
312--342 MHz band in the {\it 9th Nyquist Zone}. We are aliasing our signal on purpose, and you will need to use the actual sample
rate to accurately calculate the edges of the Nyquist window you are operating in.

Above, w also lowered the accumuation length in the SNAP spectrometer with \texttt{acc\_len}
so that, with samples coming less frequently relative to the 500-MHz sampling we used in Lab 3, we aren't waiting for too long for new
accumulations to show up.
After initialization, the only difference from the code you used in Lab 3 will be that \texttt{spec.read\_data()} returns
...
\item Use \texttt{sdr.capture\_data(...)} or \texttt{spec.read\_data(...)} to take and record data to a file for each pointing/observing mode,
along with all of the appropriate metadata for this position. Make sure you can easily reconstruct which pointing this file
corresponds to, along with all of the relevant LO/noise settings you used.
