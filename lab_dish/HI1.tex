...
\subsection {Using \texttt{screen} at the heiles computer at Leuschner}
For observing sessions that run automatically for long periods of
time, we will use \texttt{screen} to serve up a remote desktop. 'texttt{screen} is a command that allows us to maintain a persistent session on a remote machine even when everyone is logged out. If everyone uses the same \texttt{screen} session, we avoid having more than one group observing at a time. We have set up a \texttt{screen} window on the heiles computer at Leuschner.  

To attach to this window, in heiles, run 
\begin{verbatim}
    screen -r
\end{verbatim} (``r'' for ``reattach'') on heiles. This connects you to the existing session and command-line environment. Please remember to do this as soon as you log in! It can be easy to forget to go into \texttt{screen} before running your scripts as you usually would.
...
\begin{verbatim}
    Ctrl-a d
\end{verbatim}. Before you log out of heiles, it's best to first detach the screen window before exiting -- otherwise, you may close the entire screen session and accidentally abort running programs.
...
% XXX provide follow or ra/dec functionality?
\begin{enumerate}
...
% XXX need to get the LO control at leuschner migrated to ugradio.agilent
%\begin{enumerate}
