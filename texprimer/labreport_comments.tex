...
%\includegraphics[scale=.7]{simple.pdf}
%\includegraphics[scale=.8]{nicer.pdf}
\includegraphics[scale=.55]{simple.pdf}
\includegraphics[scale=.6]{nicer.pdf}
\end{center}
...
\item When plotting datapoints, it's usually a good idea to plot the
  points themselves, which you'd do with the keyword \verb$psym=4$, for
  example \footnote{Or some other number---the number gives the plot
    symbol shape.}.  Sometimes you also want to connect the datapoints
  with lines, which you do with \verb$psym=-4$.
...
\item When you do a least squares fit, you want to overplot the
  ...
  (e.g.\ {\tt plot, x, data, psym=-4}) and then use
  \verb$oplot$. Overplots are often done in color (e.g.\ {\tt oplot, x,
    fit, color=!red}), but almost 20\% of humaan males are red/green
  colorblind, so out of consideration you should overplot with a
  different line style (e.g.\ {\tt oplot, x, fit, linestyle=3)} or both
  ({\tt oplot, x, fit, color=!red, lines=3}).
