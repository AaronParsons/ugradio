...
if keyword_set(noise_off) then set_noise, IP_ADDR, PORT, noise=0, verbose=set_verbose
if keyword_set(noise_on) then set_noise, IP_ADDR, PORT, noise=1, verbose=set_verbose
...
set_dist, IP_ADDR, PORT, '1', DISTANCE = '5450000', SUCCESS=set_suc, VERBOSE=set_verbose
set_tries=set_tries+1
endwhile
set_suc=0
set_tries=0
while ((set_suc EQ 0) and (set_tries LE max_set_tries)) do begin
set_dist, IP_ADDR, PORT, '2', DISTANCE = '8200000', SUCCESS=set_suc, VERBOSE=set_verbose
set_tries=set_tries+1
endwhile
...
;print, 'in dish.pro loop control_stat port 1'
...
mv_tries=0
while ((mv_ready NE 'R') and (mv_tries LE max_mv_tries)) do begin
;print, 'in dish.pro loop control_stat port 2'
...
alpha=atan((-cos((az_rad-az_off)))/(tan(alt_rad)))
beta=asin((sin((az_rad-az_off))*cos(alt_rad)))
