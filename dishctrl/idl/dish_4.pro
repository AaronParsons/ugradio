PRO DISH, alt=alt, az=az, home=home, diode_on=diode_on, diode_off=diode_off, verbose=verbose, move_check=move_check, exit_status=exit_status

exit_status=0
set_verbose=0 					;
if keyword_set(verbose) then set_verbose=1 	;verbose value to be passed to the different functions
...
if keyword_set(diode_on) then begin 	;turns on noise diode
						
endif						

if keyword_set(diode_off) then begin	;turns off noise diode
...
if keyword_set(home) then begin		;Start of Homing
...
wait,2
go_home, IP_ADDR, PORT, verbose=set_verbose	;Actually sends the command to the controller to go home
home_ready=''
max_hm_tries=120
hm_tries=0
...
hm_tries=hm_tries + 1
endwhile
print,'dish zeroed properly'		;End of zero finding - not end of home

drive_off, IP_ADDR, PORT, verbose=set_verbose
drive_on, IP_ADDR, PORT, verbose=set_verbose

set_suc=0				;Start of going to zenith
max_set_tries=3
set_tries=0
...
set_go, IP_ADDR, PORT, VERBOSE=set_verbose	;Actually sends go command to controller
...
enc_pos, IP_ADDR, PORT, '1', ENC_POSITION = enc_1_pos, VERBOSE=set_verbose	;captures encoder position
