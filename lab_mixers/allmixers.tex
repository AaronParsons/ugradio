...
% XXX don't understand this paragraph
...
\item The output is a function of frequency, so you have to specify the
  ...
  just under $\nu_{samp}$. Thus, you calculate a {\it voltage} spectrum
...
the \verb$conj$ function, i.e.\ {\tt PF = EF * EF.conj()}.  Should the
...
\verb$PF = float(PF)$. For arrays, this becomes {\tt PF = PF.astype(numpy.float)}.
