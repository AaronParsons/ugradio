...
\item {\it The First Week. For class on 24 Jan:} Finish \S
  ...
  IDL during your presentation.}

\item {\it The Second Week. For class on 31 Jan:} Finish \S
  \ref{mixersect} and the reading in \S \ref{hetereo}. Again, be prepared
  to strut your stuff to the class.

\item {\it The Third Week. For class on 7 Feb:} Read the handouts in \S
...
in the Linux operating system, the Emacs editor, and IDL. To this end,
you'll need to become familiar with the following handouts and IDL
procedures:
...
\item Getting into IDL: {\tt idltut1\_ay121.pdf} {\it ``Quick IDL
  Tutorial Number One for AY121''} Gives you the basics of IDL.

\item Plotting in IDL: {\tt bpidl.pdf} {\it ``BPIDL---BASIC PLOTTING IN
  IDL: PLOTS, MULTIPLE PLOTS, COLORS, MAKING POSTSCRIPT FILES''} Sections
...
\end{enumerate}
%
\subsubsection{IDL Procedures}
The  following IDL procedures are needed or useful for this
lab: \begin{enumerate}
...
IDL, with the command {\tt firstN= samples[0:N-1]}), with $N$ being a
...
\item Plot a histogram of the sampled voltages (see IDL's {\tt
  histogram} function---for documentation, type {\tt ?histogram} in IDL;
  or use our convenient home-grown `wrapper' {\tt histo\_wrap}---for
  documentation, type {\tt doc, 'histo\_wrap'} \ \ in IDL). The histogram
...
\item The output is a function of frequency, so you have to specify the
  ...
  use the {\tt doc\_library} or {\tt doc} function; in IDL,
  type:\ \ {\tt doc, 'dft'}.
...
Try IDL's FFT and compare it to your DFT calculation above. The FFT
...
\item You'll need to show plots into your lab report. To do this you
  make PostScript files of your plots. See {\tt bpidl.pdf} {\it ``BPIDL---BASIC PLOTTING IN
  IDL: PLOTS, MULTIPLE PLOTS, COLORS, MAKING POSTSCRIPT FILES''} Section 6.0.1.
