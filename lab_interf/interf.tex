...
least-squares fitting and Fourier transforms to solve for the positions
of our antennas to high precision, and measure 
the angular diameters of the Sun using Bessel functions.
This lab runs for four weeks, but with robotic telescopes to control,
threaded data collection to code, and fringes to tame, this is perhaps
the hardest lab of the course. Don't fall behind!
...
\item Obtain horizon-to-horizon data on the Sun.

\item VLBI nonlinear least-squares fringe fitting to determine the most
  accurate baseline lengths.

\item Use nonlinear least-squares fitting to obtain an accurate diameter
  for the Sun. This illustrates the general Fourier transform
...
\item {\it Week 1:} Each {\it group} uses rotation
  matrices to calculate when objects of interest are up. Observe the
  Sun for a short time to confirm that it sees fringes,
  and then do a horizon-to-horizon observation, which
  requires writing an observing script to run automatically.

Each {\it person} calculates Fourier power spectra of the Sun data,
calculating the range of expected
...
\item {\it Week 2:} Each {\it group} (re)observes
  horizon-to-horizon the Sun with the
  interferometer.  Each {\it person} derives accurate antenna positions
  from the data using least-squares fitting.

\item {\it Spring Break}

\item {\it Week 3:} Finish the Sun observations.
  Each {\it person} derives angular diameters for the
  Sun using least-squares fitting as outlined in \S \ref{mun}.
...
\section{Software for Precession, Tracking, and Finding the Sun}
...
ssh pi@10.32.92.201
\end{verbatim}
If you'd like, you may use {\tt ssh} key exchange and {\tt .ssh/config} directives
to allow you to {\tt scp} data from the RPi directly to your computer. Otherwise,
you'll need to upload data to the cloud from the RPi.
...
\noindent
Our interferometer operates at about 10.5 GHz,
but the exact frequency is set by your choice of frequency for
the two local oscillators. The first one (in the neighborhood of 8.75 GHz,
+12 dBm) goes to a DSB mixer that converts RF down to an intermediate 
frequency (IF). The second (in the neighborhood of 1.54 GHz, +8 dBm)
is a SSB mixer converting IF to baseband for sampling. There is some
flexibility for where to set the first mixer (RF to IF). The second LO
must match the IF filters. Look at the system diagram in
{\tt ugradio/lab\_interf} for details.
...
%
%\noindent
%Although our sensitivity is limited, it is sometimes possible to 
%measure 'point' sources
...
\noindent
Sources of cm-wave radiation.  M17 (``M'' for Messier) and Orion
...
\noindent
Observe
the Sun for a short time---say an hour. This will give you
...
answer.
%Also, during this first week, do the horizon-to-horizon track of one of the sources from the Table.
...
will map the Sun, which never gets very far from $\delta =
...
represents neither, because neither the Sun does not have uniform
...
\noindent
Our goal is to measure the diameter of the Sun.
We'll make the assumption that the sources are uniformly-bright
...
using Fourier techniques. Some types of sources (like the event
horizon of nearby supermassive black holes!) are so small that mapping
