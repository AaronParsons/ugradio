PRO hp_dvm, mode=mode, range=range, zero=zero, reading=reading, $
            take=take, verbose=verbose  ;, reading1=reading1, reading2=reading2
...
;HOST = '10.32.92.87'
PORT = 1234       
;gpib_cmd, host, port, addr, 
;reading= gpib_cmd( host, port, addr17, '')
...
;;;if keyword_set(take) then reading2= gpib_cmd( host, port, addr17, dvm_com)
;;;reading=reading1
if keyword_set(take) and keyword_set( verbose) then print,('Measured Value = ' + STRCOMPRESS(string(reading),/remove))
if keyword_set(take) and keyword_set( verbose) then reading=reading[0]
endelse
