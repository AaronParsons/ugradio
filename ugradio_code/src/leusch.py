...
class TelescopeDirect:
    def __init__(self, serialPort='/dev/ttyUSB0', baudRate=9600, timeout=1, verbose=True,
            ...
            el_enc_offset=EL_ENC_OFFSET, el_enc_scale=EL_ENC_SCALE, dish_el_offset=DISH_EL_OFFSET):
    ...
    def move_el(self, dishEl): 
        ...
        curEl = self.get_el()
        curEl = ((float(curEl[1])-(self.dish_el_offset*(2.0**14)/(2.0*math.pi))+2.0**14) % 2.0**14)*(2.0*math.pi)/(2.0**14)
