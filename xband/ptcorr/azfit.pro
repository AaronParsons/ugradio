...
pro azfit,key,delta,alt,az,reverse,moon,kill,dial,skew,sigma,points,plotit=plotit,ewd=ewd,constant=constant
...
;    s[1,*]=rev*cos(talt*!dtor)
        ...
        dial=a[0]
        if keyword_Set(constant) then skew=0.01 else skew=a[1]
...
;do it in a way that works for more than one day
            morndex=where(taz le 180.,morncount)
            if morncount ne 0 then begin
...
;HELLO THERE HERE IS WHERE I INSERTED THE ALT STUFF
