...
pro aafit,key,delta,alt,az,reverse,moon,kill,a,sigma,points,plotit=plotit,ewd=ewd,constant=constant,aac=aac
...
;do it in a way that works for more than one day
            ...
            if rk ne 0 then begin
                junk=where(morndex eq revidx,junkey)
                if junkey ne 0 then mard=morndex[junk]
            endif
            if fk ne 0 then begin
                junk=where(morndex eq foridx,junkey)
                if junkey ne 0 then mafd=morndex[junk]
            endif
            endif
...
;            help,faketdeltaf
            if (fk and rk) then begin
                ...
                oplot,talt[foridx],tdelta[foridx],color=red,ps=2
                oplot,faketalt,faketdeltar,color=blue
                oplot,talt[revidx],tdelta[revidx],color=blue,ps=2
                if keyword_set(mafd) then $
                  oplot,talt[mafd],tdelta[mafd],color=magenta,ps=2 
                if keyword_set(mard) then $
                  oplot,talt[mard],tdelta[mard],color=cyan,ps=2 
