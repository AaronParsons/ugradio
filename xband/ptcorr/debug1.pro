...
pro errors,dir,out,i=i,silent=silent,plotit=plotit,verbose=verbose,nobase=nobase,order=order,oldschool=oldschool,savename=savename,usecor=usecor,mixcor=mixcor,scoop=scoop,errors,expected,moon,reverse,kill
...
maxrms=.01 ;the biggest number of standard deviations away before killing points
...
base_gauss2,head,dat,fit,order=order,nobase=nobase,usecor=usecor,oldschool=oldschool,scoop=scoop
...
;usecor=0
if (keyword_set(usecor) or keyword_set(mixcor)) then begin ;RETURN DIFFERENCE WRT CORRECTIONS
...
;SCOOPINESS
    ...
    plot,coord[nra,*],pow[*,nra],/ys,/xs,xtitle=strn(nra)+' - ' +titlestr[nra];  ,yr=[mm[0],mm[1]]
...
;NON SCOOPY
    if keyword_set(scoop) eq 0 then begin
        for nra=0,3 do begin
            ...
            plot,bigc[nr,nra,*],bigp[nr,nra,*],/ys,/xs,xtitle=strn(nra)+' - ' +titlestr[nra]
...
;SCOOPINESS
        for nra=0,3 do begin
                                ...
                                ;mm=minmax([mm1,mm2])
                                ;plot,fit[nra].slinfit
            plot,bigc[nr,nra,*],bigp[nr,nra,*],/ys,/xs,xtitle=strn(nra)+' - ' +titlestr[nra] ;  ,yr=[mm[0],mm[1]]
...
;interactive mode - lets user add points to the kill file
    ...
    while none eq 0 and keyword_set(i) eq 1 do begin
        ...
        if (none eq 0) and (undo eq 0) and (besilent eq 0) then print,'Adding record '+strn(key)+' of data set '+strn(nr)+' to the kill file.  Any others?'
        if undo eq 1 then print,'All four of these sub windows have been removed from the kill file.'
