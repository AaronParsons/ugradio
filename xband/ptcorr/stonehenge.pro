...
pro stonehenge,fname,key,out,allplots=allplots

plotty=keyword_Set(allplots)
...
if keyword_set(key) eq 0 then key=[0,1,2,3] else key=reform(key)
print,key
for nq= 0,n_elements(key)-1 do begin
print,nq
count=key[nq]
...
if plotty then begin
    plot,a,c,ps=4,xtitle='difference v/s alt'
    r=get_kbrd(1)
    plot,b,c,ps=4,xtitle='difference v/s az'
    r=get_kbrd(1)
endif
