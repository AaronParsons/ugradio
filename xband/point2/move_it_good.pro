...
if (alt_e EQ -2.0) then $
	str = str + '' $
else $
	if (alt_e GT -1.0) then $
		str = str + ' alt_e=' + strtrim(alt_e, 2)

if (az_e EQ -2.0) then $
	str = str + '' $
else $
	if (az_e GT -1.0) then $
		str = str + ' az_e=' + strtrim(az_e, 2)

if (alt_w EQ -2.0) then $
	str = str + '' $
else $
	if (alt_w GT -1.0) then $
		str = str + ' alt_w=' + strtrim(alt_w, 2)

if (az_w EQ -2.0) then $
	str = str + '' $
else $
	if (az_w GT -1.0) then $
		str = str + ' az_w=' + strtrim(az_w, 2)


;  SEND MESSAGE TO PC
if (VERBOSE_) then print, 'Message to pc:  ', str

senddish='/home/global/instrument/senddish'

IF NOT(NOSEND_) THEN BEGIN
	spawn, 'echo '+str+' | '+senddish, result
ENDIF ELSE $
