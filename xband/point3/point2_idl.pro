...
;
; CALLING SEQUENCE:
;	Result = POINT2([/stow] [/maint] [/home] [/pos] [/reverse] [/forward]
;			[/nosend] [/nocorrect] [/verbose] [/help] [/stop]
;			[alt=] [az=] [/noeast] [/nowest])
;
; INPUTS:
...
;	verbose:    Prints out usefule (?) program information.
...
;       move_it_idl was also modified to use idlpc.so by ES on april 3,
;       2001
...
if keyword_set(stow) then begin
    msg = move_it_idl(POINT, 90.0, 180.0, 90.0, 180.0, noeast = noeast,$
                  nowest = nowest)
...
if keyword_set(maint) then begin
    msg = move_it_idl(POINT, 11.0, 180.0, 11.0, 180.0, noeast = noeast,$
                  nowest = nowest)
    if (not(msg_check(msg)) OR VERBOSE_) then $
      print, msg
    return, 0
...
if keyword_set(stop) then begin
    msg = move_it_idl(TRACK, 0.0, 0.0, 0.0, 0.0, noeast = noeast,$
                  nowest = nowest)
    if (not(msg_check(msg)) OR VERBOSE_) then $
      print, msg
    return, 0
...
if keyword_set(home) then homer
...
;	Point those dishes!!!
    ...
    msg = move_it_idl(POINT, alt_e, az_e, alt_w, az_w, noeast = noeast,$
                  nowest = nowest)
    if (not(msg_check(msg)) OR VERBOSE_) then $
      print, msg
    return, 0
...
if keyword_set(pos) then begin
    msg = move_it_idl(POINT, -1.0, -1.0, -1.0, -1.0, noeast = noeast,$
