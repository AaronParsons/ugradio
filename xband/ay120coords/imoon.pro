...
;	/elong - if not specified assumes campbell, elong=-122.25730
;       
;
...
pro imoon,first,second,aa=aa,ut=ut,date=date,hms=hms,verbose=verbose, $
	debug=debug,geocentric=geocentric,getlst=getlst, juldate=juldate

;TAKE CARE OF ELONG...
if (n_elements( elong) eq 0) then elong=-122.25730
...
;IF THE JULIAN DAY IS SPECIFIED VIA DATE AND UT, THEN CALCULATE IT...
IF (KEYWORD_SET( DATE) OR KEYWORD_SET( UT) ) THEN BEGIN
...
ENDIF ELSE BEGIN
    if keyword_Set(ut) then darr=makedate(ut) else $
      darr=makedate()
ENDELSE
;GET THE JULIAN DATE (no longer reduced!)
...
if (verby) then print,'hehe',ra,dec
;GET ALT AND AZ
;lst=ilst(date=date,time=ut,verbose=verbose,local=local)
ct2alst,lst, elong, nothinghere,jd  
    getlst=lst
    ha=lst-ra
