...
;       juldate=juldate - specify julian day, overrides date/ut
...
;
;	JD= xxxxx: specify the nonreduced UT julian day. returns ra and dec, 
;	even if the /aa option is specified. [isun, ra, dec, jd=xxx]
;	to get current UT jd: jd= systime( /julian, /ut)
;
...
pro isun,first,second,aa=aa,ut=ut,date=date,rise=rise,set=set, $
	hms=hms,verbose=verbose,debug=debug,getlst=getlst,getjd=getjd, $
	jd=jd, juldate=juldate

if keyword_set( jd) then begin
;print, jd
	sunpos, jd, ra, dec
	ra= ra/15.d
;	print, ra, dec, '  returning'
;	wait,.01
	first=ra
	second=dec
	return
...
;DEFAULT IS JULIAN DAY IS CURRENT ONE...
if n_elements( jd) eq 0 then jd= systime( /julian, /utc)

;IF THE JULIAN DAY IS SPECIFIED VIA JULDATE, THEN SET IT...
if (n_elements (juldate) ne 0) then jd=juldate

;MAKE A DATE ARRAY
if n_elements(ut) ne 0 then darr=makedate(ut,date)
...
;juldate,darr,jd
;getjd=jd
;if verby then print,'reduced julian:',jd
